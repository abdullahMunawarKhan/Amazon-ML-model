...
The runnable implementation is under `src/`. It uses only the supplied TSV files:
there are no external lookups, APIs, registries, or internet augmentation. Source 2
and Source 3 are indexed in SQLite to avoid a full cross-join under a 16 GB RAM budget.

Create an environment and install the pinned dependencies from `requirements.txt`,
...
The baseline prints a held-out, Source-1-level macro F_0.5 score before inference and
writes `output/candidate_pairs.tsv` and `output/matching_results.tsv`. To train the
feature-based matcher instead:

```bash
python run_pipeline.py --mode xgboost
```

The matcher is XGBoost 2.1.3, licensed Apache-2.0, with far fewer than 8B parameters.
The pipeline keeps country as an open string value and does not use an allowlist; test
countries unseen in training are handled by the same equality feature.
